	...
	*/
	...
	function plugin_row_meta_filter($plugin_meta, $plugin_file) {
		if ($plugin_file == $this->plugin_basename) {
			...
			if (is_blog_admin())
				$title = __('Active Modules: ', 'seo-wizard');
			else
				$title = '';
	...
	function add_postmeta_box() {
		...
		foreach ($posttypes as $screen) {
			...
			//	add_meta_box('su_postmeta', __('Link Settings', 'seo-update'), create_function('', 'global $seo_update; $seo_update->show_postmeta_box("'.$screen.'");'), $screen, 'normal', 'high');
		}
	}
	...
	function show_postmeta_box($screen) {
		...
		if ( !$this->is_sdf_active() ) {
			...
			printf(__('%1$s %2$s by %3$s', 'seo-wizard'),
				'<a href="'.SU_PLUGIN_URI.'" target="_blank">'.__(SU_PLUGIN_NAME, 'seo-wizard').'</a>',
				SU_VERSION,
				'<a href="'.SU_AUTHOR_URI.'" target="_blank">'.__(SU_AUTHOR, 'seo-wizard').'</a>'
			);
			echo '</p>';
		}
	...
	function tabs($tabs=array(), $table=false, &$callback=null) {
		...
		if ($c = count($tabs)) {
			...
			if ($c >= 1) {
				echo "\n\n</div>\n";
				echo "\n\n</div>\n";
              echo  '<p><a href="http://seo.uk.net/?wizard" target="_blank"><img src="http://seo.uk.net/wp-content/uploads/2014/10/seo-banner.gif" /></a></p>';
			}
		}
	}
